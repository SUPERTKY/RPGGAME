...
startTurnPhase(firstMove) {
    ...
    // 先行チームの左端のキャラを取得
    let firstMoveCharacters = Object.values(this.players).filter(p => p.team === firstMove);
    firstMoveCharacters.sort((a, b) => a.joinTime - b.joinTime); // 先に参加した順に並べる
    this.currentAttackerIndex = 0; // 左端のキャラから開始
...
startPlayerTurn() {
    ...
    if (!currentAttacker) {
        console.error("❌ 攻撃キャラが見つかりません");
        return;
    }

    console.log(`⚔️ ${currentAttacker.name} のターン開始！`);

    // 「考え中」テキストをキャラの上に表示
    this.thinkingText = this.add.text(currentAttacker.sprite.x, currentAttacker.sprite.y - 60, "考え中", {
        fontSize: "18px",
        fill: "#ffff00",
        stroke: "#000000",
        strokeThickness: 4
    ...
    // 技選択ウィンドウを表示
    this.displaySkillSelection(currentAttacker);

    // 制限時間（例: 10秒）を設定
    this.turnTimer = this.time.delayedCall(10000, () => {
        console.warn("⏳ ターン制限時間終了");
        this.nextTurn();
    });
}

getCurrentAttacker() {
    let attackTeam = this.players.filter(p => p.team === this.currentTurnTeam);
    if (this.currentAttackerIndex >= attackTeam.length) {
        return null;
    }
    return attackTeam[this.currentAttackerIndex];
}

displaySkillSelection(player) {
    console.log("🛠️ 技選択ウィンドウ表示");

    const skillBoxX = 100; // 左側に表示
    const skillBoxY = this.scale.height / 2;
    
    // フレーム（背景）
    this.skillFrame = this.add.rectangle(skillBoxX, skillBoxY, 180, 100, 0x333333, 0.8);
    this.skillFrame.setOrigin(0, 0.5);

    // 技の情報
    this.skillText = this.add.text(skillBoxX + 10, skillBoxY - 30, "A", {
        fontSize: "20px",
        fill: "#ffffff",
        stroke: "#000000",
        strokeThickness: 3
    ...
    this.mpText = this.add.text(skillBoxX + 10, skillBoxY, `MP: 1`, {
        fontSize: "18px",
        fill: "#ffffff",
    });

    this.damageText = this.add.text(skillBoxX + 10, skillBoxY + 30, `DAMAGE: 5`, {
        fontSize: "18px",
        fill: "#ff4444",
    });

    // クリックで技を実行
    this.skillFrame.setInteractive();
    this.skillFrame.on("pointerdown", () => {
        this.executeSkill(player, 5);
    });
}

executeSkill(attacker, damage) {
    console.log(`🔥 ${attacker.name} が攻撃！ ダメージ: ${damage}`);

    // ターゲット（仮：敵の左端をターゲット）
    let targetTeam = this.players.filter(p => p.team !== attacker.team);
    targetTeam.sort((a, b) => a.joinTime - b.joinTime); // 先に入った順
    let target = targetTeam[0]; // 左端の敵
    ...
    // 一瞬赤くするエフェクト
    let originalTint = target.sprite.tint;
    ...
    // HPが 0 以下なら戦闘不能処理（後で追加可能）
    if (target.hp <= 0) {
        console.log(`💀 ${target.name} が戦闘不能！`);
    }
...
nextTurn() {
    ...
    let attackTeam = this.players.filter(p => p.team === this.currentTurnTeam);
    if (this.currentAttackerIndex >= attackTeam.length) {
        console.log("🔄 チームの全員が攻撃終了、次のチームへ");
        // TODO: チームの切り替え処理を後で追加
        return;
    }
